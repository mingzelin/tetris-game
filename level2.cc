...
Block* Level2::generateNext(std::vector<std::vector <Cell>> &grid,int &success,char &k,char &next,bool init,bool norand,std::string norandfilename){

    if(next=='A'){
        rand_result=rand() % 7; //range is from 0-6
        if(rand_result==0){
            next= 'S';
        }
